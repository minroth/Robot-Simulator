...
// プログラム 2.10: プログラム2.5の描画なし版 hopper3.cpp by Kosei Demura (2007-5-17)
//
...
// hopper3.cpp by Kosei Demura (2007-5-18)
//
// 更新履歴　(change log)
...
int main (int argc, char *argv[])
{
  ...
  dsSimulationLoop(argc,argv,640,480,&fn); // 描画をしないのでコメントアウト
  /* int i = 0;
  while (i < 10000) {
		i++;
    simLoop(0); // シミュレーションループ，引数は0
    const dReal *p = dBodyGetPosition(torso.body);
		// 胴体の重心位置表示[m]
    //printf("STEPS:%5d pos: x=%.3f y=%.3f z=%.3f\n", STEPS, p[0], p[1], p[2]);
  } */
  dWorldDestroy(world);
  dCloseODE();
